...
exports.createProposal = async (req, res) => {
  try {
    ...
    if (clientCheck.length === 0) {
      return res.status(400).json({ status: "Failure", message: "Invalid client_id: Client does not exist." });
    }
    ...
    const result = await runQuery(q, [
      client_id, txn_id, proposal_type, billing_type, 
      final_start_date || null, final_end_date || null, 
    ...
    res.status(200).json({ status: "Success", message: "Proposal created", proposalId: result.insertId });
  } catch (error) {
...
exports.updateProposal = async (req, res) => {
  try {
    ...
    const { 
      client_id,
      proposal_type, billing_type, billing_start_date, billing_end_date,
      sections_json, optional_toggles, pricing_table_json, grand_total_excl_gst, 
      terms_notes_json, notes_json, additional_remarks, client_instructions, updated_by 
    } = req.body;
    ...
    if (existingProformas.length > 0) {
      ...
      for (const prof of existingProformas) {
        const gst_amt = prof.is_gst && (Buffer.isBuffer(prof.is_gst) ? prof.is_gst[0] === 1 : Number(prof.is_gst) === 1) ? Number((base_amt * prof.gst_rate) / 100).toFixed(2) : 0;
...
exports.sendProposalToClient = async (req, res) => {
  try {
    ...
    res.status(500).json({ status: "Failure", message: "Server error while sending proposal" });
  }
};
  ...
  try {
    ...
    try {
      ...
      const insertResult = await runQuery(paymentQ, [
        proforma_id, proposal_id, client_id, amount, is_gst ? 1 : 0, tds_applicable ? 1 : 0, tds_percentage || 0, tds_amount || 0, final_amount, payment_date, payment_mode, transaction_reference, remark, created_by || "System", invoice_txn_id, realized_ad_budget || 0, realized_google_budget || 0, realized_meta_budget || 0
      ]);
      ...
      try {
        ...
        const pendingOutstanding = Math.max(previousBalance - Number(amount || 0), 0);
        ...
        const notifyRes = await notificationService.dispatch({ event: "PAYMENT_ALERT", payload });
      ...
      res.status(500).json({ status: "Failure", message: "Error processing payment transaction" });
    }
  } catch (error) {
...
exports.approvePayment = async (req, res) => {
  try {
    ...
    try {
      ...
      const totalPastPayments = Number(pastPayments[0].total_past || 0);
      const previous_amt = totalPastPayments === 0 ? 0 : Math.max(Number(proforma.total_amount) - totalPastPayments, 0);
      const current_amt = totalPastPayments === 0 ? Math.max(Number(proforma.total_amount) - received_amt, 0) : Math.max(previous_amt - received_amt, 0);
      ...
      await runQuery(insertInvoiceQ, [
        bill_type, newBillNumber, invoice_txn_id, client_id, client.client_name || '', client.client_organization || '',
        client.email || '', client.phone || '', client.address || '', proposal.created_by || '', proposal.billing_start_date || '', proposal.billing_end_date || '',
        payment.payment_mode || 'UPI', payment.payment_date || null, payment.transaction_reference || null, '', '',
        'pending', received_amt, current_amt, previous_amt, tds_amt,
        createdAt, 'proposal', proforma.id, proforma.base_amount || 0, proforma.gst_rate || 0, proforma.gst_amount || 0, payment.realized_ad_budget || 0, payment.realized_google_budget || 0, payment.realized_meta_budget || 0
      ]);
      ...
      for (const item of items) {
        ...
        if (isComplimentary) {
          ...
          `;
          await runQuery(igQ, [invoice_txn_id, client_id, item.service_name || '', item.category_name || '', null, item.editing_type_name || '', item.unit_price || 0, item.quantity || 1, '0', '0', 0, item.total_price || 0, '', proposal.created_by || '', createdAt]);
        }
      }
      ...
      try {
        ...
        const payload = {
          ...
          adminTimeStr: new Date().toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })
        };
        const notifyRes = await notificationService.dispatch({ event: "FINAL_INVOICE", payload });
        if (!notifyRes.error) deliveryStatus = notifyRes;
      } catch (err) {
        console.error("Auto notification error:", err);
      }
      ...
      res.status(500).json({ status: "Failure", message: "Error approving payment" });
    }
  } catch (error) {
...
exports.generateInvoiceFromProforma = async (req, res) => {
  try {
    ...
    if (paymentRows.length === 0) {
      return res.status(400).json({ status: "Failure", message: "No payment records found for this proforma" });
    }
    ...
    let newBillNumber = "01";
    if (lastBillRows.length > 0 && lastBillRows[0].bill_number) {
      const lastNum = parseInt(lastBillRows[0].bill_number.split("-").pop(), 10);
    ...
    const invoiceSource = proforma.source_type === 'manual' ? 'manual' : 'proposal'; // Not inserted directly but conceptually useful
    ...
    const startDateFallback = proforma.duration_start_date ? safeDate(proforma.duration_start_date) : null;
    const endDateFallback = proforma.duration_end_date ? safeDate(proforma.duration_end_date) : null;

    const dgEmployeeFallback = proforma.source_type === 'manual'
      ? (req.user?.name || "System")
      : (proposal.created_by || null);

    const insertValues = [
      invoiceSource, proforma.proposal_id || null, proforma.id,
      bill_type, newBillNumber,
      proforma.base_amount || null, proforma.gst_rate || null, proforma.gst_amount || null,
      txn_id, proforma.client_id,
      client.client_name || null, client.client_organization || null,
      client.email || null, client.phone || null, client.address || null,
      dgEmployeeFallback,
      startDateFallback,
      endDateFallback,
      ...
      proforma.total_amount || null, received_amt,
...
async function createProposalPdfBuffer(id, snapshotData = null) {
  try {
    ...
    pricing.forEach(item => {
      const isAds = item.source === "custom_ads" || item.service_type === "Ads Campaign" || item.service_name === "Ads Campaign" || item.source_type === "ads_campaign";
      
      if (isAds) {
        const explicitBudget = item.budget !== undefined ? item.budget : (item.ad_budget !== undefined ? item.ad_budget : (item.amount !== undefined ? item.amount : null));
    ...
    const propType = proposal.proposal_type === 'digital_marketing' ? 'Digital Marketing Proposal For' : 'Development Proposal For';
    const duration = cover.duration || '1 Month';
    const propDate = cover.proposal_date || new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' });
    ...
    // 2. Executive Summary
    if (isSectionIncluded("executive_summary") && hasRenderableContent(sections.executive_summary)) {
    ...
    if (isSectionIncluded("about_us") && hasRenderableContent(sections.about_us)) {
      htmlContent += `<h2>ABOUT US</h2>`;
      htmlContent += textToHtml(valueToText(sections.about_us));
    }

    // 4. Understanding Client's Problem
    if (isSectionIncluded("client_problem") && hasRenderableContent(sections.client_problem)) {
      htmlContent += `<h2>UNDERSTANDING CLIENT'S PROBLEM</h2>`;
      htmlContent += textToHtml(valueToText(sections.client_problem));
    }

    // 5. Proposed Solution
    if (isSectionIncluded("proposed_solution") && hasRenderableContent(sections.proposed_solution)) {
    ...
    // 7. Strategy Overview
    if (isSectionIncluded("strategy_overview") && hasRenderableContent(sections.strategy_overview)) {
      htmlContent += `<h2>STRATEGY OVERVIEW</h2>`;
      htmlContent += textToHtml(valueToText(sections.strategy_overview));
    }

    // 8. Timeline & Milestones
    if (isSectionIncluded("timeline") && Array.isArray(sections.timeline) && sections.timeline.length > 0) {
    ...
    // 9. Expected Results
    if (isSectionIncluded("expected_results") && hasRenderableContent(sections.expected_results)) {
      htmlContent += `<h2>EXPECTED RESULTS</h2>`;
      htmlContent += textToHtml(valueToText(sections.expected_results));
    }

    // 10. Pricing & Investment
    if (isSectionIncluded("pricing_investment") && pricing && pricing.length > 0) {
      ...
      const finalGrandTotalForPdf = Math.max(0, dmSubtotal - discountAmt) + adsSubtotal;
    ...
    // 12. Terms & Conditions
    if (isSectionIncluded("combined_notes_tc")) {
      const rawTerms = hasRenderableContent(sections.terms_conditions) ? sections.terms_conditions : savedTerms;
      const termsList = Array.isArray(rawTerms) ? rawTerms : (hasRenderableContent(rawTerms) ? [valueToText(rawTerms)] : []);
    ...
    const additionalRemarks = sections.additional_remarks || proposal.additional_remarks;
    if (isSectionIncluded("additional_remarks") && hasRenderableContent(additionalRemarks)) {
      htmlContent += `<h2>ADDITIONAL REMARKS</h2>`;
      htmlContent += textToHtml(valueToText(additionalRemarks));
    }

    // 14. Client Instructions
    const clientInstructions = sections.client_instructions || proposal.client_instructions;
    if (isSectionIncluded("client_instructions") && hasRenderableContent(clientInstructions)) {
      htmlContent += `<h2>CLIENT INSTRUCTIONS</h2>`;
      htmlContent += textToHtml(valueToText(clientInstructions));
    }

    // 15. Why Choose Us
    if (isSectionIncluded("why_choose_us") && hasRenderableContent(sections.why_choose_us)) {
...
exports.getRevenueHistory = async (req, res) => {
  try {
    ...
    res.status(200).json({ status: "Success", totals: { totalInvoiced, totalReceived, totalPending, totalTds }, invoices });
  } catch (error) {
