...
const logPublicAccess = async (doc_type, doc_id, req, status) => {
  try {
    ...
    const q = `
      INSERT INTO public_access_logs (doc_type, doc_id, ip_address, user_agent, status) 
      VALUES (?, ?, ?, ?, ?)
    `;
    await queryDb(q, [doc_type || 'unknown', doc_id || null, ip, userAgent, status]);
  } catch (error) {
 ...
 * Overwrites any existing token hash for the same invoice, invalidating old links.
 */
exports.generatePublicAccessToken = async (client_id, txn_id, doc_type, snapshotJson = null) => {
  const secret = process.env.PUBLIC_LINK_SECRET;
  if (!secret) throw new Error("PUBLIC_LINK_SECRET is not defined in .env");

  const token = jwt.sign({ client_id, txn_id, doc_type }, secret, { expiresIn: "30d" });
  ...
  if (finalSnapshot) {
    try {
      ...
      finalSnapshot = "GZIP:" + zlib.gzipSync(Buffer.from(finalSnapshot)).toString('base64');
    } catch(e) {
  ...
  if (doc_type === "proposal") {
    const q = `
      ...
      WHERE id = ?
    `;
    // For proposals, txn_id acts as proposal id in this context
    await queryDb(q, [tokenHash, mysqlDatetime, finalSnapshot, txn_id]);
  } else {
    const table = doc_type === "proforma" ? "proposal_proforma" : "invoice";
    const q = `
      UPDATE ${table} 
...
const validateTokenAndGetPayload = async (token, expectedDocTypes = []) => {
  ...
  if (expectedDocTypes.length > 0 && !expectedDocTypes.includes(decoded.doc_type)) {
  ...
  if (decoded.doc_type === "proposal") {
    q = "SELECT public_token_hash, public_snapshot_json FROM proposals WHERE id = ? LIMIT 1";
    params = [decoded.txn_id]; // txn_id acts as proposal id
  } else {
    const table = decoded.doc_type === "proforma" ? "proposal_proforma" : "invoice";
  ...
  if (results[0].public_token_hash !== tokenHash) throw new Error("Token revoked or overwritten");

  return { decoded, record: results[0] };
};

/**
 * Endpoint to get all invoice data for the public page
 */
exports.getPublicInvoiceData = async (req, res) => {
  ...
  try {
    // Validate token
    const { decoded } = await validateTokenAndGetPayload(token, ["invoice", "proforma", "quotation", "final"]);
    ...
    const [
      ...
      // Client Data
      queryDb("SELECT * FROM invoice WHERE client_id = ? AND txn_id = ?", [client_id, txn_id]),
      
      // We will check for calculator services. If we also need to check revenue_engine services, we could.
      // But AdminInvoice usually fetches these standard tables for calculator invoices.
      // (Wait, AdminInvoice fetches service data. It seems invoice itself is the main source)
      queryDb("SELECT * FROM revenue_engine_invoice WHERE client_id = ? AND txn_id = ?", [client_id, txn_id]).catch(() => []), // just in case
      
      // Graphic Services
      queryDb("SELECT * FROM invoice_graphic WHERE txn_id = ? AND client_id = ?", [txn_id, client_id]),
      
      // Ads Campaigns
      queryDb("SELECT * FROM ads_campaign_details_invoice WHERE txn_id = ? AND client_id = ?", [txn_id, client_id]),
      
      // Complimentary
      queryDb("SELECT * FROM complimentary_invoice WHERE txn_id = ? AND client_id = ?", [txn_id, client_id]),
      
      // Additional Services
      queryDb("SELECT * FROM addtional_service WHERE txn_id = ? AND client_id = ?", [txn_id, client_id]),
      
      // Remaining Amount
      queryDb("SELECT * FROM amount_remaining WHERE txn_id = ? AND client_id = ?", [txn_id, client_id]),
      
      // Client Notes
      queryDb("SELECT * FROM invoice_client_notes WHERE client_id = ? AND txn_id = ?", [client_id, txn_id]),
      
      // Discounts
      queryDb("SELECT * FROM discount WHERE client_id = ? AND txn_id = ?", [client_id, txn_id]),
      
      // Proforma Payments (only if doc_type is proforma)
      doc_type === "proforma" 
        ? queryDb("SELECT * FROM proposal_payment_records WHERE client_id = ? AND txn_id = ? AND status = 'approved'", [client_id, txn_id])
        : Promise.resolve([])
    ]);
...
exports.getPublicInvoicePdf = async (req, res) => {
  ...
  try {
    ...
    // "The existing /generate-invoice endpoint takes client-generated HTML and has no auth.
    // The new /api/public/invoice/:token/pdf endpoint validates the public token and generates HTML server-side — the client never needs to send HTML."
    ...
    if (!htmlContent) {
      return res.status(400).json({ status: "Failure", message: "HTML content missing" });
    }
    ...
    if (error.message === "Document not found" || error.message === "Invalid or expired token" || error.message === "Token revoked or overwritten" || error.message === "Invalid document type for this endpoint") {
      return res.status(403).json({ status: "Failure", message: error.message });
    }
    console.error("[PUPPETEER ERROR]", error);
    if (!res.headersSent) {
      res.status(500).json({ status: "Failure", message: "Server failed to process PDF layout." });
    }
  }
}
...
exports.getPublicProposalData = async (req, res) => {
  const { token } = req.params;
  try {
    const { decoded, record } = await validateTokenAndGetPayload(token, ["proposal"]);
    ...
    if (record.public_snapshot_json) {
      try { 
        ...
        if (jsonStr.startsWith("GZIP:")) {
          const zlib = require('zlib');
          jsonStr = zlib.gunzipSync(Buffer.from(jsonStr.substring(5), 'base64')).toString('utf-8');
        }
...
exports.getPublicProposalPdf = async (req, res) => {
  ...
  try {
    ...
    const pdfBuffer = await proposalController.createProposalPdfBuffer(decoded.txn_id, snapshot);
